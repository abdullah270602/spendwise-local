...
A loan is settled by attaching the entry that repaid it, not by recording an amount beside it. The ledger takes an entry out of the month by its `debt_id` and by nothing else, so a settlement with no entry attached — which is the right record for cash, since banknotes send no alert — leaves a bank repayment counted as income while the loan reads as closed. `settleDebt` therefore takes an optional transaction id, and every screen that can see a candidate offers it: the entry suggests the loan it looks like, the loan lists the entries that look like it, and Review asks per loan before it asks anything else. A loan is therefore a principal with a stack of settlements against it rather than a thing that is open or closed, and the entry screen draws it that way: principal, returned, still out, and the instalments that brought it. Those instalments are read back off the entries rather than out of `debt_settlements`, which no view model surfaces -- a settlement made from an entry carries that entry's whole amount, and the leg that opened a loan moves the opposite way to every payment into it, which is the same test the matcher applies. Only what was typed in as a bare amount has no entry, and `settledByHand` is what carries it. Matching is deliberately narrow and never acts alone — right direction, right currency, not before the loan existed, never more than is still out, and then either the counterparty's name in the narration or exactly the outstanding amount — because a loan the owner believes is closed is money they will never ask for again.
...
A third contract governs colour itself, and it is two choices rather than one. A `SpendWisePalette` is what the user picks; a `Ground` is what the brightness preference resolves — graphite, or the warm paper the PDF exporter has always printed on. `SpendWiseColors` flattens the pair into the statics that widgets and painters read directly, because a screen that had to ask which ground it was on at every call site would be a worse app than one that is simply repainted. The stored palette is always its graphite form, since that is its identity and what the picker names; `SpendWiseColors.lit` is how a screen asks for the version actually being drawn. The paper twin is derived — cap lightness at 0.40 and lift saturation a quarter, which is the exporter's own `paperTone`, then darken until the tone clears 4.5:1 against paper — except Slate, which is hand-built, because every one of its tones is above that cap and so the cap is the end of the palette rather than a relight of it.

Two numbers in the app encode "away from the ground", and both invert when the ground does: the Home ribbon's branch alphas and the floor in `SpendWiseColors.category()` that stops a long list fading out. A partial alpha walks a tone toward what is behind it, which on graphite is away from the text and on paper is straight at it, so both belong to the `Ground` and not to the widget that spends them. Two consequences follow for anything that draws. A `ThemeData` is only half of how the app is coloured, so building one applies its ground rather than trusting a caller to have done it. And every `CustomPainter` mixes in `GroundAware`: a painter compares the values it was handed, none of them was ever handed a colour, and five returned `false` outright — correct until the ground could move underneath one, at which point the ribbon goes on painting graphite onto paper and the app looks broken rather than re-themed.

The home-screen widget adds the only other place data leaves the ledger, and it is deliberately the narrowest boundary in the app. A widget runs in its own process at times the app is not alive, so it can never hold the SQLCipher key; instead the app *publishes* what the drawing needs — a flag, one or two ratios, and the current palette's colours — into plain unencrypted `SharedPreferences`, and the widget draws from that alone. Nothing that crosses is an amount, a balance or an account name: `HomeWidgetSnapshot`'s equality is defined so that a small month and a huge one with the same split compare equal, which is the proof that only the ratio survives the trip. The widget therefore shows the shape of a month and tells a stranger glancing at the phone nothing at all. The figure is redrawn natively in Kotlin rather than rendered from Flutter, because a widget resized or restored by Android has no live engine to render with; `flow_shape_geometry_ports_test.dart` reads both files as text and fails, naming the constant, when the two copies of the geometry drift apart. The colours published to it are always the graphite palette, whichever ground the app itself is on: a widget sits on somebody's wallpaper rather than on either of ours, and the renderer answers that by outlining every shape with a two-tone keyline instead of by knowing what is behind it.
