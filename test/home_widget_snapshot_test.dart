...
void main() {
  ...
  group('keptFraction', () {
    ...
    test(
      'an overspent month divides on magnitude, the same way FlowShape '
    ...
    test(
      '"Only what I can spend" takes saved money out of the shape, '
    ...
    test(
      '"Saving gets its own branch" leaves the headline fraction alone, '
    ...
    test('"Marked inside what is still yours" and "a seam in the shape" are '
  ...
  group('hasSavedBranch and savedOfKept', () {
    test(
      'only "siblings" ever draws a third branch -- every other style, '
    ...
    test('"siblings" divides the kept branch by the same share FlowShape '
      ...
      final snapshot = HomeWidgetSnapshot.from(month, HomeSavingsStyle.siblings);
      expect(snapshot.hasSavedBranch, isTrue);
      expect(snapshot.savedOfKept, closeTo(100000 / 250000, 1e-9));
    });

    test(
      'a month that saved nothing draws no branch even under "siblings" -- '
  ...
  group('the boundary', () {
    ...
    test(
      'the same holds once a third branch is in play -- the saved share '
